...
- Run `pnpm run prep` before handing off significant changes; use `pnpm run prep:push` (or set `HOOK_RUN_WEB_BUILD=1`) prior to publishing or pushing.
...
| Full validation  | `pnpm run prep:push` (format → clean → build → lint → typecheck:all → test)     |
