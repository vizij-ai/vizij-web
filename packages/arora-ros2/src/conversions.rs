...
use ros2_client::{Name, Node, DEFAULT_SUBSCRIPTION_QOS};
...
pub type SlotValueStream = Pin<
    Box<dyn Stream<Item = Result<HashMap<std::string::String, Value>, std::string::String>> + Send>,
>;
...
pub fn setup_slot_subscriber(
    ...
    match value_type {
        Type::F64 => {
            setup_typed::<msg_types::Float64>(node, &topic_name, path, |msg| Value::F64(msg.data))
        }
        Type::F32 => {
            setup_typed::<msg_types::Float32>(node, &topic_name, path, |msg| Value::F32(msg.data))
        }
        Type::I64 => {
            setup_typed::<msg_types::Int64>(node, &topic_name, path, |msg| Value::I64(msg.data))
        }
        Type::I32 => {
            setup_typed::<msg_types::Int32>(node, &topic_name, path, |msg| Value::I32(msg.data))
        }
        Type::U64 => {
            setup_typed::<msg_types::UInt64>(node, &topic_name, path, |msg| Value::U64(msg.data))
        }
        Type::U32 => {
            setup_typed::<msg_types::UInt32>(node, &topic_name, path, |msg| Value::U32(msg.data))
        }
        Type::Boolean => {
            setup_typed::<msg_types::Bool>(node, &topic_name, path, |msg| Value::Boolean(msg.data))
        }
        Type::String => {
            setup_typed::<msg_types::String>(node, &topic_name, path, |msg| Value::String(msg.data))
        }
        other => {
            ...
            setup_typed::<msg_types::String>(node, &topic_name, path, |msg| {
...
pub async fn drive_slot_streams(mut streams: Vec<SlotValueStream>, handler: SetSlotValuesHandler) {
    info!(
        "drive_slot_streams: merging {} subscription streams",
        streams.len()
    );
...
fn setup_typed<M: MessageType>(
    ...
    let stream = unfold((subscription, 0u32), move |(sub, consecutive_errors)| {
        ...
        let log_topic = log_topic.clone();
        async move {
            match sub.async_take().await {
                Ok((msg, _info)) => {
                    let value = convert(msg);
                    debug!("Received on {}: {:?}", log_topic, value);
