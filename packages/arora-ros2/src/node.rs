...
use log::{info, warn};
use ros2_client::{
    AService, Context, ContextOptions, Name, Node, NodeName, NodeOptions, ServiceMapping,
...
impl AroraConnection for AroraRos2Node {
    ...
    fn run(
        ...
        Box::pin(async move {
            ...
            let ctx = Context::with_options(ContextOptions::new().domain_id(self.domain_id))
                .map_err(|e| format!("failed to create ROS2 context: {e:?}"))?;

            let node_name = NodeName::new(&format!("/{}", self.namespace), "vizij_node")
            ...
            for (info, handler) in methods.iter() {
                let service_name = format!("/{}/methods/{}", self.namespace, info.path);
                match spawn_method_service(&mut node, &service_name, handler.clone()) {
                    Ok(abort) => {
                        info!("ROS2 service: {}", service_name);
                        abort_handles.push(abort);
                    }
                    Err(e) => {
                        warn!("Failed to create service for method '{}': {}", info.path, e);
                    }
                }
            }
...
fn spawn_method_service(
    ...
    let server = node
        ...
        .map_err(|e| format!("failed to create service '{service_name}': {e:?}"))?;
