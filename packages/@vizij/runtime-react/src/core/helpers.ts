/**
 * Framework-free helpers for the Vizij face runtime.
 *
 * Moved verbatim out of `VizijRuntimeProvider.tsx` as the first strangler
 * step of the `@vizij/face-core` extraction
 * (docs/redesign/06-track-2-implementation.md §3.1). Nothing here may import
 * React or the render store.
 */
import type {
...
import { collectInputPaths } from "../utils/graph";
...
export function resolveEasing(easing?: AnimateValueOptions["easing"]) {
...
export function findRootId(world: Record<string, any>): string | null {
...
export function normaliseBundleKind(kind: unknown): string {
...
export function addConstraintVariant(
...
export function stripRigFacePrefix(path: string): string {
...
export function namespaceSubscriptions(
...
export function namespaceGraphSpec(
  spec: GraphRegistrationConfig["spec"],
  namespace: string,
...
export function pickBundleGraph(
...
export function extractIrGraph(payload: unknown): IrGraph | undefined {
...
export function convertBundleGraph(
...
export type GraphNodeSpec = NonNullable<
  GraphRegistrationConfig["spec"]["nodes"]
>[number];
export type GraphEdgeSpec = NonNullable<
  GraphRegistrationConfig["spec"]["edges"]
>[number];

export function stripVizijMetadata(
  spec: GraphRegistrationConfig["spec"],
): GraphRegistrationConfig["spec"] {
  if (!spec || typeof spec !== "object") {
    return spec;
...
export function extractVizijInputMetadata(
  spec: GraphRegistrationConfig["spec"],
): VizijInputMetadata[] {
  if (!spec || typeof spec !== "object") {
    return [];
...
export function convertBundleAnimations(
...
export type ExtractedAnimationTrack = {
  componentId?: string;
  component?: string;
...
export type ExtractedAnimationClip = {
  id?: string;
  name?: string;
...
export function mergeAnimationLists(
...
export function extractProgramResetValues(
  value: unknown,
...
export function mergeProgramLists(
...
export function normalizeStoredAnimationInterpolation(
  interpolation: unknown,
): "linear" | "step" | "cubic" {
  const mode =
...
export function buildStoredAnimationTransitions(
...
export const IMPLICIT_PAUSE_GAP_S = 5;

export const inferImplicitPause = (dt: number): number => {
