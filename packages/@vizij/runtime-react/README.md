...
- `orchestratorBackend="aroraWeb"` defaults to the composed Arora module path loaded through `/arora-web/modules/manifest.json`; `@vizij/orchestrator-react` derives dependency preloads from the selected Arora module header's `imports` and resolves them through its Arora module registry. Pass `orchestratorInitInput={{ orchestratorModule: "compatibility" }}` to use the compatibility Arora module, `moduleRegistryUrl` for another manifest, `moduleRegistry` to add custom imported modules, or `preloadModules` when you need to override header-derived preloads.
