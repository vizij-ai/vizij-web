...
| **rig** ⚠️⚠️                         | **Six meanings.** (1) the rig graph (a GraphSpec); (2) rig inputs (the controls); (3) rig kind (generic vs. face-specific); (4) pose rig (the pose system); (5) `RigControllerProvider` (the state provider); (6) the `rig/{faceId}/` path prefix. **VIZ-80 exists to fix this** |
