...
**Storybook ↔ Figma mode sync (done):** a custom manager panel ([`.storybook/manager.tsx`](../../apps/vizij-authoring/.storybook/manager.tsx)) replaces the stock addon-designs panel (which only reads `parameters.design`, never globals). It reads the story's `design.url` (light node) **and** the `theme` global; in Dark it swaps the node-id for a per-component **dark-mode tile** in Figma (a frame with the `Theme` collection's Dark mode applied). So toggling Storybook Light/Dark drives the embedded Figma "Design" tab to the matching mode — **light ↔ light, dark ↔ dark, automatically.** (`addon-designs` is dropped from `main.ts` addons to avoid a duplicate static panel.)

**Dark mode via Figma variables + modes (done):** rather than duplicate `theme` variants, the library uses a **`Theme` variable collection with `Light`/`Dark` modes** (Professional plan supports up to 4 modes). 24 semantic color variables mirror `styles.css` (`bg/app`, `bg/surface`, `text/primary`, `border/default`, `accent/default`, `accent/fg`, `danger/*`, plus `success|warning|info|danger` tint sets). Every component's fills/strokes are **bound** to these variables (role-aware: white-on-teal → `accent/fg`, white knobs/checks left literal, chip tints → functional vars). Each component exists **once** (real variant props only — `variant`/`tone`/`state`; the temporary `theme` variants were removed and the converted singles dissolved back to standalone components, keeping their node ids so `parameters.design` + `.figma.tsx` still resolve). Two preview frames — **`Theme Preview — Light`** and **`Theme Preview — Dark`** (background bound to `bg/app`, the dark one set to the `Dark` mode) — hold instances of all 22 components, giving a true dark backdrop behind everything and switching the whole set with one mode toggle. Net: **light ↔ light and dark ↔ dark**, single source of truth, matched to the code tokens.
