...
A and B are balanced middles — A gives the clearest *learning* narrative, B gives
the clearest *role separation* — but each carries a structural tax (A: cross-stage
...
*substrate* decision, while D's strength (canvas UX) and A's strength (lifecycle
narrative) are *app-shell* decisions. They compose. That is the hybrid.
...
three *already assume this substrate*. Adopt C's versioning discipline
...
**Mitigating C's "reference app rots into a demo" risk:** we don't ship C's *thin
reference app* — we ship D's *designed* canvas app as the flagship. The app is a
...
*least risky migration* because today's app already has the viewport +
...
biggest tension (cross-stage tasks strain hard modes) by demoting it from *modes*
...
4. **How far to push "Behavior" representation.** Ship the node-graph editor as-is
   (Track 2 · R3), or invest in the keyboard-first rule-list alt-view (D §3, area
   5) sooner — it is a user-facing accessibility win that could move into Track 1?
