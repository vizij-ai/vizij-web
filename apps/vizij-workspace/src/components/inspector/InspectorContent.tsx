...
type PoseVariableItem =
    ...
    | { type: "color"; label: string; featureId: string; components: { varId: string; poseVal: number, channel: 'r' | 'g' | 'b' }[] };
...
export function InspectorContent() {
    ...
    if (selectedPoseId) {
        ...
        if (pose) {
            ...
            const groupedVariables = (() => {
                const targetToFeature: Record<string, { featureId: string, featureKey: string, objectId: string, objectName: string, componentKey: string }> = {};
                ...
                const groups: Record<string, { label: string, items: PoseVariableItem[] }> = {};
                const colorFeatures: Record<string, { label: string, featureId: string, groupKey: string, components: { varId: string, poseVal: number, channel: 'r' | 'g' | 'b' }[] }> = {};

                Object.entries(pose.values).forEach(([varId, val]) => {
                    const mInput = managedStandardInputs.find(m => m.input.id === varId);
                    ...
                    for (const [targetId, binding] of Object.entries(bindings)) {
                        if (binding.inputId === varId || (binding.slots && binding.slots.some(s => s.inputId === varId))) {
                    ...
                    if (featureInfo && featureInfo.featureKey.toLowerCase() === 'color') {
                        ...
                        if (!colorFeatures[colorKey]) {
                            colorFeatures[colorKey] = { label: "Color", featureId: featureInfo.featureId, groupKey, components: [] };
                        }
                        const channel = featureInfo.componentKey.toLowerCase() as 'r' | 'g' | 'b';
                        colorFeatures[colorKey].components.push({ varId, poseVal: val, channel });
                    } else {
                        if (!groups[groupKey]) groups[groupKey] = { label: groupLabel, items: [] };
                        groups[groupKey].items.push({ type: "scalar", varId, poseVal: val });
                    }
                });

                Object.values(colorFeatures).forEach(cf => {
                    if (!groups[cf.groupKey]) groups[cf.groupKey] = { label: cf.label, items: [] };
                    groups[cf.groupKey].items.push({ type: "color", label: cf.label, featureId: cf.featureId, components: cf.components });
                });
            ...
            return (
                <div className="flex flex-col gap-2 p-2 min-h-0 flex-1">
                        ...
                        {groupedVariables.map(group => (
                            <div key={group.label} className="flex flex-col gap-2">
                                <div className="flex items-center gap-2 px-1 py-1 border-b border-white/5">
                                    ...
                                    <span className="text-[10px] font-bold text-slate-300 uppercase tracking-widest">{group.label}</span>
                                </div>
                                        ...
                                        if (item.type === "scalar") {
                                            ...
                                            return (
                                                <RiggingPropertyRow
                                                    ...
                                                    onSaveToDefault={() => updatePoseValue(pose.id, varId, inputValues[varId] ?? 0)}
                                                    onScrubStart={() => { scrubValuesRef.current[varId] = inputValues[varId] ?? 0; }}
                                                    ...
                                                    renderMainInput={() => (
                                                        <div className="flex items-center gap-2 flex-1 group/row">
                                                            <input
                                                                ...
                                                                value={liveVal}
                                                                className="flex-1 h-1 bg-slate-700/50 rounded-lg appearance-none cursor-pointer accent-blue-500 min-w-0"
                                                                onChange={(e) => handleInputValueChange(varId, parseFloat(e.target.value))}
                                                            />
                                                            <div className="w-12 flex-shrink-0">
                                                                <input
                                                                    type="text"
                                                                    ...
                                                                    className={cn(
                                                                        ...
                                                                        isDifferent ? "text-blue-400 font-bold" : "text-slate-400"
                                                                    )}
                                                                    onChange={(e) => {
                                                                        const v = parseFloat(e.target.value);
                                                                        if (!isNaN(v)) handleInputValueChange(varId, v);
                                                                    }}
                                                                />
                                                            </div>
                                                        </div>
                                                    ...
                                                    renderDefaultInput={() => (
                                                        <div className="flex items-center gap-2 flex-1 group/row">
                                                            ...
                                                            <Button
                                                                ...
                                                                onClick={(e) => { e.stopPropagation(); removePoseInput(pose.id, varId); }}
                                                            >
                                                                <Trash2 size={12} />
                                                            </Button>
                                                        </div>
                                            ...
                                            const isDifferent = item.components.some(c => Math.abs((inputValues[c.varId] ?? 0) - c.poseVal) > 0.001);
                                            ...
                                            const renderColorInputs = (isPoseValue: boolean) => {
                                                const curR = isPoseValue ? (r?.poseVal ?? 0) : (inputValues[r?.varId ?? ''] ?? 0);
                                                const curG = isPoseValue ? (g?.poseVal ?? 0) : (inputValues[g?.varId ?? ''] ?? 0);
                                                const curB = isPoseValue ? (b?.poseVal ?? 0) : (inputValues[b?.varId ?? ''] ?? 0);
                                                const hex = rgbToHex(curR, curG, curB);

                                                return (
                                                    <div className="flex items-center gap-2 flex-1 group/row">
                                                        <Popover className="relative flex items-center">
                                                            <PopoverButton className="w-8 h-4 rounded border border-slate-700 shadow-sm transition-transform hover:scale-105" style={{ backgroundColor: hex }} />
                                                            <PopoverPanel anchor="bottom end" className="flex flex-col gap-2 p-2 bg-slate-900 border border-slate-800 rounded-lg shadow-xl z-[100] mt-1">
                                                                <HexColorPicker color={hex} onChange={(h) => {
                                                                    const rgb = hexToRgb(h);
                                                                    if (rgb) handleBulkChange(isPoseValue, rgb.r, rgb.g, rgb.b);
                                                                }} />
                                                            </PopoverPanel>
                                                        </Popover>
                                                        <div className="flex gap-1 flex-1 min-w-0">
                                                            {[{ c: r, ch: 'R' as const }, { c: g, ch: 'G' as const }, { c: b, ch: 'B' as const }].map(({ c, ch }) => (
                                                                <div key={ch} className="flex-1 flex items-center bg-slate-950/40 rounded border border-slate-800/50 px-1 py-0.5">
                                                                    <ScrubbableLabel label={ch}
                                                                        onScrub={(delta, totalDelta) => {
                                                                            if (c?.varId) {
                                                                                const step = 0.01;
                                                                        ...
                                                                        onScrubStart={() => {
                                                                            if (c?.varId) {
                                                                                const baseline = isPoseValue ? c.poseVal : (inputValues[c.varId] ?? 0);
                                                                                scrubValuesRef.current[c.varId] = baseline;
                                                                            }
                                                                        }}
                                                                        className={cn("text-[9px] font-bold mr-1", ch === 'R' ? 'text-red-500' : ch === 'G' ? 'text-green-500' : 'text-blue-500')}
                                                                    />
                                                                    <input type="text" value={(isPoseValue ? (c?.poseVal ?? 0) : (inputValues[c?.varId ?? ''] ?? 0)).toFixed(2)}
                                            ...
                                            return (
                                                <RiggingPropertyRow
                                                    ...
                                                    defaultLabel="Pose"
                                                    hasDifferentDefault={isDifferent}
                                                    onResetToDefault={() => item.components.forEach(c => handleInputValueChange(c.varId, c.poseVal))}
                                                    onSaveToDefault={() => item.components.forEach(c => updatePoseValue(pose.id, c.varId, inputValues[c.varId] ?? 0))}
                    ...
                    <Button variant="secondary" size="sm" className="w-full mt-2 gap-2 border-slate-800 bg-slate-800/40 hover:bg-slate-800/60" onClick={() => setShowSelector(true)}>
                        <Plus size={14} /> Add Variable
                    </Button>
                    <Modal open={showSelector} onClose={() => setShowSelector(false)} title="Select Variable" maxWidth="md">
                        <VariableSelector onSelect={handleAddVariable} onCancel={() => setShowSelector(false)} />
                    </Modal>
                </div>
            );
        }
    ...
    // 3. Rig Mode
    if (selectedRigId) {
        const rigInput = managedStandardInputs.find(m => m.input.id === selectedRigId);
        if (rigInput) {
            ...
            const dependents = (() => {
                ...
                Object.entries(bindings).forEach(([targetId, binding]) => {
                    const isDriven = binding.inputId === selectedRigId || (binding.slots && binding.slots.some(s => s.inputId === selectedRigId));
                    if (isDriven) {
                        let label = targetId;
                        for (const obj of objects) {
                            for (const feat of obj.features) {
            ...
            return (
                <div className="p-2 flex flex-col gap-4">
                    ...
                    <RiggingPropertyRow
                        label="Current Value"
                        onScrubStart={() => { scrubValuesRef.current[input.id] = value; }}
                        onScrub={(_, totalDelta) => {
                        ...
                        renderMainInput={() => (
                            <div className="flex items-center gap-2 flex-1">
                                <input type="range" min={input.range.min ?? -1} max={input.range.max ?? 1} step={0.01} value={value}
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        {dependents.length === 0 ? (
                            <div className="text-xs text-slate-500 italic px-2">Not driving any scene properties</div>
                        ) : (
                            <div className="flex flex-col gap-1 max-h-[200px] overflow-y-auto custom-scrollbar bg-slate-900/40 rounded p-1 border border-slate-800/50">
                                {dependents.map(d => (
                                    <div key={d.targetId} className="text-xs text-slate-300 p-1.5 hover:bg-slate-800/50 rounded flex items-center gap-2">
    ...
    return (
        <div className="flex flex-col items-center justify-center h-48 text-slate-500 text-xs gap-3 border border-dashed border-slate-800/50 rounded-xl bg-slate-900/20 m-1">
            <p className="font-medium text-slate-400">Select an item to inspect details</p>
        </div>
    );
}
