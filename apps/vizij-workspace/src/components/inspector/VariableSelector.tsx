...
export function VariableSelector({ onSelect, onCancel }: VariableSelectorProps) {
    const [activeTab, setActiveTab] = useState<"variables" | "scene">("variables");
        ...
        <div className="flex flex-col h-[500px] w-full bg-slate-900 text-slate-200 overflow-hidden rounded-xl border border-slate-800 shadow-2xl">
            ...
            <div className="p-3 border-b border-slate-800 flex flex-col gap-3 bg-slate-900/50 backdrop-blur-md">
                ...
                <div className="relative group">
                    ...
                    <Input
                        placeholder={activeTab === "variables" ? "Search variables..." : "Search scene..."}
...
function VariablesList({ search, onSelect }: { search: string, onSelect: (s: VariableSelection) => void }) {
    const { managedStandardInputs, bindings } = useBindingAuthoring(s => s);
    const { objects } = useSceneComposer();

    const groupedVariables = useMemo(() => {
        const q = search.toLowerCase();
        const filtered = managedStandardInputs.filter(m =>
            m.input.id.toLowerCase().includes(q) ||
            (m.input.label && m.input.label.toLowerCase().includes(q))
        );

        const groups: Record<string, { label: string, vars: typeof managedStandardInputs }> = {};
    ...
    if (groupedVariables.length === 0) {
        return <div className="p-8 text-center text-xs text-slate-500 italic">No variables found</div>;
    }
                        ...
                        {group.vars.map(item => (
                            ...
                            >
                                ...
                                <div className="flex flex-col min-w-0">
                                    <span className="text-xs font-semibold truncate group-hover:text-blue-200">{item.input.label || item.input.id}</span>
                                    <span className="text-[10px] text-slate-500 font-mono truncate">{item.input.id}</span>
                                </div>
                            </div>
                        ))}
                    </div>
...
function SceneTree({ search, onSelect }: { search: string, onSelect: (s: VariableSelection) => void }) {
    ...
    const matchingIds = useMemo(() => {
        ...
        objects.forEach(obj => {
            if (obj.name.toLowerCase().includes(q) || obj.type.toLowerCase().includes(q)) {
                matches.add(obj.id);
            }
        });
        return matches;
    ...
    const renderNode = (nodeId: string, depth: number) => {
        ...
        const hasMatchingChild = !search || objects.some(o =>
            matchingIds.has(o.id) && o.parentId === nodeId
        );
                        ...
                        {hasFeatures && (
                            <button
                                ...
                                className={cn(
                                    ...
                                    showFeatures ? "bg-blue-600 text-white" : "bg-slate-800 text-slate-500 hover:bg-slate-700 hover:text-slate-300"
                                )}
                            >
                                <Zap size={8} className={showFeatures ? "text-white" : "text-blue-500/70"} />
                                Props
                            </button>
                        )}
                    </span>
                ...
                >
                    {object.features.map(feature => (
                        ...
                        >
                            ...
                            <span className="text-[11px] font-medium truncate">{feature.label}</span>
                        </div>
                    ))}
                </div>
            )}
