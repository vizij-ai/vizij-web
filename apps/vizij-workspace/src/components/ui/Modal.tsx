...
export function Modal({
  ...
  return (
    <BaseDialog.Root open={open} onOpenChange={(isOpen) => !isOpen && onClose()}>
      <BaseDialog.Portal>
        <BaseDialog.Backdrop
          className="fixed inset-0 z-50 bg-zinc-950/80 backdrop-blur-md transition-all duration-300 data-[state=open]:opacity-100 data-[state=closed]:opacity-0"
        />
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-0 overflow-y-auto" onClick={(e) => e.target === e.currentTarget && onClose()}>
          ...
          >
            <header className="flex justify-between items-center px-6 py-4 bg-bg-panel/50 border-b border-border-default/50">
              <BaseDialog.Title
                className="m-0 text-lg font-bold text-text-primary uppercase tracking-widest font-heading"
              >
                {title}
              </BaseDialog.Title>
              <BaseDialog.Close
