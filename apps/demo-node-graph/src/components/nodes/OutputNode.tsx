...
const OutputNode = ({ id, data }: NodeProps<{ label?: string; inputs?: string[] }>) => {
  ...
  useEffect(() => {
    setHistory(prevHistory => {
        ...
        (data.inputs ?? []).forEach(inputHandle => {
            const edge = edges.find(e => e.target === id && e.targetHandle === inputHandle);
            if (edge) {
  ...
  return (
    <div style={{ padding: "15px 20px", background: "#2a2a2a", borderRadius: 8, border: "1px solid #555", minWidth: 250, position: "relative" }}>
      ...
      {(data.inputs ?? []).map((inputHandle, i) => {
        ...
        return (
          <div key={inputHandle} style={{ position: "relative", marginBottom: 15 }}>
            <Handle type="target" id={inputHandle} position={Position.Left} style={{ ...handleStyle, top: i * 80 + 25 }} />
      ...
      <button onClick={addInput} style={{width: '100%', marginTop: 10}}>+ Add Input</button>
    </div>
  );
};
