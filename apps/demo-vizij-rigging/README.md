...
Author high-level emotion rigs on top of low-level Vizij mappings. Load a Vizij GLB, import the low-level graph spec exported from `demo-vizij-authoring`, sculpt per-input neutral values, capture emotions, blend them live, and export the resulting high-level graph plus rig configuration for reuse.
