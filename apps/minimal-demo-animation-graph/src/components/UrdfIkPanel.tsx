  ...
  return (
    ...
    >
      <header style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        <h3 style={{ margin: 0, color: minimalDemoTheme.text }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 16 }}>
        ...
        >
          <span style={{ fontSize: 13, color: minimalDemoTheme.muted }}>
            URDF XML
          </span>
          <textarea
            value={urdfText}
      ...
      >
        <strong style={{ color: minimalDemoTheme.text, fontSize: 14 }}>
          Solver joints (latest evaluation)
        </strong>
        {jointList.length === 0 ? (
          <span style={{ color: minimalDemoTheme.muted, fontSize: 13 }}>
          ...
          <ul style={{ margin: 0, paddingLeft: 20, display: "grid", gap: 4 }}>
            {jointList.map(({ name, value }) => (
              <li
                key={name}
                style={{ color: minimalDemoTheme.text, fontSize: 13 }}
              >
                <code style={{ color: minimalDemoTheme.muted }}>{name}</code>:{" "}
                {value.toFixed(3)} rad
              </li>
            ))}
          </ul>
