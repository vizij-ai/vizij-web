...
# Launch the demo
pnpm --filter minimal-demo-animation-graph dev
```
...
```bash
pnpm --filter minimal-demo-animation-graph dev            # start Vite dev server
pnpm --filter minimal-demo-animation-graph build          # type-check & build production bundle
```
