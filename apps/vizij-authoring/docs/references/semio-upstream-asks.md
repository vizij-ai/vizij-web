...
| [STUDIO-143](https://linear.app/semio-ai/issue/STUDIO-143) | `Autocomplete`/`Combobox` popup width needs a viewport ≥768px **tall**                                      | `ui/Combobox.tsx` forces `popupClassName="w-[var(--anchor-width)]!"`. Without it the popup renders wider than its own modal.                                                                                                |
| [STUDIO-144](https://linear.app/semio-ai/issue/STUDIO-144) | `popupClassName` lands on a `position:static` element, so it cannot reach the positioner's hardcoded `z-50` | `ui/Combobox.tsx` resolves its own `portalContainer` from the nearest `[role="dialog"]`, so the popup joins the modal's stacking context instead of losing to it at `z-50` vs `z-[4100]`.                                   |
