...
39. Inspector and pipeline authoring semantics are now split at the support/UI boundary: pipeline migration/math, binding resolution codes, face-channel value authority, pose contribution math, and rig parent-alias synchronization live in `@vizij/studio-support`; the authoring app keeps inspector labels, lock toggles, repair copy, and display formatting.
40. Pose-rig compiler and service semantics are now support-owned: rich pose IR/config types, group membership normalization, pose graph spec building/parsing, config/IR round-tripping, graph summary generation, and pose snapshot creation live in `@vizij/studio-support`; the authoring app keeps stable import adapters, store state, UI interactions, and panel orchestration.

Latest verification for this pass:

1. `pnpm --filter @vizij/studio-support lint`
2. `pnpm --filter @vizij/studio-support typecheck`
3. `pnpm --filter @vizij/studio-support test` - 46 files, 303 tests passed.
...
9. `pnpm --filter vizij-authoring test` - 80 files passed, 545 tests passed, 1 perf file/test skipped.
10. `pnpm --filter vizij-authoring test:e2e:arora` - 3 workflow tests passed in 2.5 minutes, including face load, UI-edited animation and graph execution through composed Arora runtime, and exported GLB round-trip.
...
12. `git diff --check`
13. After the compile-state adapter cleanup, `pnpm --filter vizij-authoring exec vitest --run src/state/__tests__/stores.test.ts src/components/app/Viewer.test.tsx src/hooks/__tests__/useAnimationTransport.test.tsx` - 36 tests passed, with `pnpm --filter vizij-authoring typecheck` and `pnpm --filter vizij-authoring lint`.
