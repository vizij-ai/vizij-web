...
4. Stage 0 (`A0.1`-`A0.3`) is complete and Stage 1 is in progress with `B1.1` + `B1.2` landed; next priorities are `B1.3` and `B1.4`.
...
- Status: `done`
- Latest evidence:
  - `2026-02-19 20:31Z` — `pnpm --filter vizij-authoring run validate` -> pass (`typecheck` phase exit 0).

### Lint

- Status: `done`
- Latest evidence:
  - `2026-02-19 20:31Z` — `pnpm --filter vizij-authoring run validate` -> pass (`lint` phase exit 0; 0 warnings, 0 errors).

### Test

- Status: `done`
- Latest evidence:
  - `2026-02-19 20:31Z` — `pnpm --filter vizij-authoring run validate` -> pass (`vitest` run; 66 files / 354 tests passed).
