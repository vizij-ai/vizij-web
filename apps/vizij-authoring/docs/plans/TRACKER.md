...
4. Current top priority is Stage 0 stabilization in `ROADMAP.md` (`A0.1`, `A0.2`, `A0.3`).
...
| ---- | ----------- | -------- | ----------------------------------------------------------------------------------- |
| A0.1 | in_progress | P0       | Canonical pose-weight input paths + Inputs pane synchronization is being finalized. |
| A0.2 | planned     | P0       | Import retarget sequencing/rebind hardening still open.                             |
| A0.3 | planned     | P0       | MVP pose authoring smoke coverage not yet consolidated.                             |
...
- Status: `done`
- Latest evidence:
  - `2026-02-19 03:23Z` — `pnpm --filter vizij-authoring run validate` -> pass (`typecheck` phase exit 0).

### Lint

- Status: `done` (warning-only caveat)
- Latest evidence:
  - `2026-02-19 03:23Z` — `pnpm --filter vizij-authoring run validate` -> pass (`lint` phase exit 0; 7 warnings in `VariablesPanel.tsx`, 0 errors).

### Test

- Status: `done`
- Latest evidence:
  - `2026-02-19 03:23Z` — `pnpm --filter vizij-authoring run validate` -> pass (`vitest` run; 65 files / 339 tests passed).

## Active Risks and Mitigations

1. Risk: Pose-weight input synchronization can cause update churn if create/update/delete diffing is not stable.
   - Mitigation: keep source-id-based reconciliation and add churn regression coverage under `A0.1`.
2. Risk: Import edge cases can still produce incorrect retarget ordering under complex legacy assets.
   - Mitigation: execute `A0.2` with sequencing-focused tests before release.
3. Risk: Multi-group overlap behavior can skew with additive/average policies when memberships are uneven.
