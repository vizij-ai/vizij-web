...
export function BindingConnections({
  ...
  const aggregateOutputRigs = useMemo(
    () =>
      visibleRigs.filter((rig) =>
        rig.sourceKinds.includes("pose-aggregate-output"),
      ),
    [visibleRigs],
  );
  const nonAggregateRigs = useMemo(
    () =>
      visibleRigs.filter(
        (rig) => !rig.sourceKinds.includes("pose-aggregate-output"),
      ),
    [visibleRigs],
  );
  const poseGroupAggregateRigs = useMemo(
    () =>
      nonAggregateRigs.filter((rig) =>
        rig.sourceKinds.includes("pose-group-output"),
      ),
    [nonAggregateRigs],
  );
  const topLevelVariableRigs = useMemo(
    () =>
      nonAggregateRigs.filter(
        (rig) => !rig.sourceKinds.includes("pose-group-output"),
      ),
    [nonAggregateRigs],
  );
  ...
  const poseGroups = useMemo(() => {
    ...
    >();
    const groupedPoseIds = new Set<string>();

    trace.targets.forEach((target) => {
      target.matchedPoseOutputs.forEach((output) => {
        const groupId = output.inputId || "__ungrouped";
        const standardInput = standardInputsById.get(groupId);
        const label =
          standardInput?.label ||
          standardInput?.path ||
          output.inputId ||
        ...
        if (existing) {
          existing.poseIds.add(output.poseId);
        } else {
          groups.set(groupId, {
            id: groupId,
            label,
            poseIds: new Set([output.poseId]),
          });
        }
      });
    });
    ...
    const resolvedGroups = Array.from(groups.values())
      .map((group) => {
        const poses = Array.from(group.poseIds)
          .map((poseId) => poseById.get(poseId))
          .filter((pose): pose is NonNullable<typeof pose> => Boolean(pose))
          .sort((a, b) => a.label.localeCompare(b.label));
        poses.forEach((pose) => groupedPoseIds.add(pose.id));
        return {
          id: group.id,
          label: group.label,
          poses,
    ...
    const ungroupedPoses = connections.poses
      .filter((pose) => !groupedPoseIds.has(pose.id))
      .sort((a, b) => a.label.localeCompare(b.label));
    if (ungroupedPoses.length > 0) {
      resolvedGroups.push({
        id: "__ungrouped",
        label: "Other",
        poses: ungroupedPoses,
      });
    }
  ...
  const [expandedRigCategoryIds, setExpandedRigCategoryIds] = useState<
    Set<string>
  >(() => new Set());
  const [expandedTraceTargetIds, setExpandedTraceTargetIds] = useState<
    Set<string>
  >(() => new Set());
  ...
  const togglePoseGroupExpansion = useCallback((id: string) => {
    setExpandedPoseGroupIds((current) => {
      const next = new Set(current);
      if (next.has(id)) next.delete(id);
      else next.add(id);
      return next;
  ...
  const toggleRigCategoryExpansion = useCallback((id: string) => {
    setExpandedRigCategoryIds((current) => {
      const next = new Set(current);
      if (next.has(id)) next.delete(id);
      else next.add(id);
      return next;
  ...
  const toggleTraceTargetExpansion = useCallback((id: string) => {
    setExpandedTraceTargetIds((current) => {
      const next = new Set(current);
      if (next.has(id)) next.delete(id);
      else next.add(id);
      return next;
  ...
  const renderPoseRow = useCallback(
    (pose: (typeof connections.poses)[number], className?: string) => {
  ...
  const renderRigRow = useCallback(
    (rig: (typeof visibleRigs)[number], className?: string) => {
        ...
        >
          <div className="flex items-center gap-1 group/rig">
            ...
            <Button
              ...
              onClick={() => {
                if (onSelectRig) {
                  onSelectRig(rig.id, getPrimaryRigSourceKind(rig.sourceKinds));
                  return;
                }
  ...
  return (
    <div className="flex flex-col gap-3 p-2 mt-2 border-t border-border-default/50">
      ...
      <div className="flex flex-col gap-1.5">
        {connections.poses.length > 0 && (
          <div className="flex flex-col gap-1">
            <button
              type="button"
              onClick={() => setIsPosesSectionOpen((current) => !current)}
              className="flex items-center gap-1 px-1 py-0.5 rounded text-[9px] text-slate-600 font-medium uppercase hover:bg-purple-500/10 w-fit"
              data-testid="binding-poses-section-toggle"
              aria-expanded={isPosesSectionOpen}
            >
              {isPosesSectionOpen ? (
                <ChevronDown size={12} />
              ) : (
              ...
              <span>Poses</span>
              <span className="opacity-60">({connections.poses.length})</span>
            </button>
            {isPosesSectionOpen &&
              poseGroups.map((group) => {
                const isGroupOpen = expandedPoseGroupIds.has(group.id);
                const groupTestId = `binding-pose-group-toggle-${encodeURIComponent(group.id)}`;
                return (
                  <div key={group.id} className="flex flex-col gap-1 ml-2">
                    <button
                      type="button"
                      onClick={() => togglePoseGroupExpansion(group.id)}
                      className="flex items-center gap-1 px-1 py-0.5 rounded text-[9px] text-purple-300/80 font-medium uppercase hover:bg-purple-500/10 w-fit"
                    ...
                    >
                      ...
                      <span className="opacity-60">({group.poses.length})</span>
                    </button>
                    {isGroupOpen &&
                      group.poses.map((pose) => renderPoseRow(pose, "ml-2"))}
                  </div>
                );
              })}
          </div>
        ...
        {visibleRigs.length > 0 && (
          <div className="flex flex-col gap-1">
            <button
              type="button"
              onClick={() => setIsRigsSectionOpen((current) => !current)}
              className="flex items-center gap-1 px-1 py-0.5 rounded text-[9px] text-text-muted font-medium uppercase hover:bg-accent-subtle w-fit"
              data-testid="binding-rigs-section-toggle"
              aria-expanded={isRigsSectionOpen}
            >
              {isRigsSectionOpen ? (
                <ChevronDown size={12} />
              ) : (
              ...
              <span>Rigs</span>
              <span className="opacity-60">({visibleRigs.length})</span>
            </button>
            {isRigsSectionOpen &&
              rigCategories.map((category) => {
                const isCategoryOpen = expandedRigCategoryIds.has(category.id);
                return (
                  <div key={category.id} className="flex flex-col gap-1 ml-2">
                    <button
                      type="button"
                      className="flex items-center gap-1 px-1 py-0.5 rounded text-[9px] text-text-muted font-medium uppercase hover:bg-accent-subtle w-fit"
                      data-testid={`binding-rig-category-toggle-${category.id}`}
                      aria-expanded={isCategoryOpen}
                      onClick={() => toggleRigCategoryExpansion(category.id)}
                    >
                      {isCategoryOpen ? (
                        <ChevronDown size={12} />
                      ) : (
                      ...
                      <span className="opacity-60">
                        ({category.rigs.length})
                      </span>
                    </button>
                    {isCategoryOpen &&
                      category.rigs.map((rig) => renderRigRow(rig, "ml-2"))}
                  </div>
                );
              })}
          </div>
      ...
      <div className="flex flex-col gap-1.5 pt-2 mt-1 border-t border-border-default/40">
        <button
          type="button"
          className="text-[10px] font-bold text-text-secondary uppercase flex items-center gap-1 w-fit hover:text-text-primary transition-colors"
          onClick={() => setIsTraceSectionOpen((current) => !current)}
          data-testid="binding-trace-section-toggle"
          aria-expanded={isTraceSectionOpen}
        >
          {isTraceSectionOpen ? (
            <ChevronDown size={12} />
          ) : (
        ...
        {isTraceSectionOpen && trace.targets.length === 0 ? (
          ...
          <div className="flex flex-col gap-1.5">
            {trace.targets.map((target) => {
              const isTargetOpen = expandedTraceTargetIds.has(target.targetId);
              return (
                <div
                  key={target.targetId}
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <button
                      type="button"
                      className="flex items-center gap-1 text-[10px] font-semibold text-text-primary truncate hover:text-accent transition-colors"
                      data-testid={`binding-trace-target-toggle-${encodeURIComponent(target.targetId)}`}
                      aria-expanded={isTargetOpen}
                      onClick={() =>
                        toggleTraceTargetExpansion(target.targetId)
                      }
                    >
                      {isTargetOpen ? (
                        <ChevronDown size={12} />
                    ...
                    {onSelectTarget ? (
                      <button
                        type="button"
                        className="text-[9px] font-semibold text-text-muted hover:text-accent transition-colors"
                        onClick={() => onSelectTarget(target.targetId)}
                      ...
                      <Chip tone="default">Read-only</Chip>
                    )}
                  </div>
                  {isTargetOpen && (
                    <>
