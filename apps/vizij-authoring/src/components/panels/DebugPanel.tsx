...
interface DebugInstructionTabProps {
  label: string;
  summary: string;
  icon: ReactNode;
  children: ReactNode;
...
function DebugInstructionTab({
  label,
  summary,
  icon,
  children,
  instructions,
}: DebugInstructionTabProps) {
  return (
...
interface PlaybackTabProps {
  ...
  formattedGraphTime: string;
  formattedFrameRate: string;
    ...
    <div className="flex flex-col gap-5 text-xs font-mono">
      <div className="p-4 rounded-xl bg-bg-panel border border-border-default space-y-4">
        <div className="flex justify-between items-center pb-3 border-b border-border-default">
          <span className="text-text-muted font-medium">Engine Status</span>
          <span
            className={cn(
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="space-y-1">
            ...
            <div className="h-[29px] flex items-center">
              {faceSegment ? (
                <Chip tone="info" className="w-full justify-center">
                  {faceSegment}
                </Chip>
              ) : (
                <span className="text-text-muted italic px-2">—</span>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 pt-1">
          {!graphPlaybackAvailable && (
            <div className="w-full rounded-md bg-bg-secondary/60 text-text-muted text-[11px] px-3 py-2 border border-border-default">
              Playback controls are disabled until runtime-backed transport
          ...
          <Button
            ...
            disabled={graphStatus !== "ready" || !graphPlaybackAvailable}
            size="sm"
...
export function DebugPanel({
  ...
  const formattedFrameRate = useMemo(
    () => (graphFrameRate > 0 ? `${graphFrameRate.toFixed(1)} fps` : "— fps"),
    [graphFrameRate],
  );
  ...
  }, [graphPlaybackAvailable, graphPlaybackState, pauseGraph, playGraph]);

  const tabRegistry = useMemo<Record<HealthTabId, () => ReactNode>>(
    () => ({
      playback: () => (
        <PlaybackTab
          ...
          formattedGraphTime={formattedGraphTime}
          formattedFrameRate={formattedFrameRate}
          graphPlaybackState={graphPlaybackState}
          graphPlaybackAvailable={graphPlaybackAvailable}
      ...
      diagnostics: () => (
        <DebugInstructionTab
          ...
          instructions={
            <ol className="list-decimal pl-4 space-y-1 text-text-secondary text-[11px] leading-relaxed">
              <li>
                Generate a machine report after large binding changes to capture
                slot metadata.
              </li>
              <li>
        ...
        <DebugInstructionTab
          ...
          instructions={
            <ul className="list-disc pl-4 space-y-1 text-text-secondary text-[11px] leading-relaxed">
              ...
              <li>
                This wipes stored inputs, bindings, and overrides for the
                current asset only.
              </li>
            </ul>
          }
        >
          <div className="p-4 bg-bg-panel border border-border-default rounded-xl space-y-3">
            <p className="text-xs text-text-secondary leading-relaxed">
              Clears stored overrides for the currently loaded Vizij asset. This
              is useful if authoring states become desynced.
            </p>
            <Button
              variant="danger"
              className="w-full"
    ...
    [
      bundleAudit,
      bundleAuditError,
      bundleAuditPanelStatus,
      canRunRobotDataAudit,
      faceId,
      faceSegment,
      formattedFrameRate,
      formattedGraphTime,
  ...
  const renderTabPanel = useCallback(
    (tabId: string) => {
      const renderTab = tabRegistry[tabId as HealthTabId];
      return renderTab ? renderTab() : null;
    },
    [tabRegistry],
  );
