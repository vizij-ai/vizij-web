...
export default function PersistencePanel(): JSX.Element {
  ...
  const exportSpec = useCallback(() => {
    ...
    downloadJsonFile(payload, "vizij-graph.json");
  ...
  const onFile = useCallback(
    async (f?: File) => {
      if (!f) return;
      try {
        const parsed = await readJsonFile<Record<string, unknown>>(f);
        setSpec(parsed);
      } catch (err) {
        console.error("Failed to import spec:", err);
        void showAlert("Failed to parse spec JSON. See console for details.");
      }
    },
    [setSpec, showAlert],
  );
  ...
  const saveToLocal = useCallback(() => {
    try {
      ...
      void showAlert("Graph saved to localStorage (vizij_graph_spec)");
    } catch (err) {
      console.error("Failed to save to localStorage:", err);
      void showAlert("Failed to save to localStorage. See console.");
    }
  }, [nodes, edges, nodesToSpec, showAlert]);

  const loadFromLocal = useCallback(() => {
    try {
      const txt = localStorage.getItem("vizij_graph_spec");
      if (!txt) {
        void showAlert("No saved graph found in localStorage.");
        return;
      }
      ...
      void showAlert("Failed to load from localStorage. See console.");
    }
  }, [setSpec, showAlert]);

  const loadFixture = useCallback(async () => {
    if (!selectedFixture) {
      void showAlert("Select a fixture to load.");
      return;
    }
    try {
      ...
      void showAlert("Failed to load fixture graph. See console for details.");
    }
  }, [selectedFixture, setSpec, showAlert]);
  ...
  const normalizeCurrent = useCallback(async () => {
    try {
      ...
      setSpec(normalized);
      void showAlert("Current graph normalized via wasm spec normalizer.");
    } catch (err) {
      console.error("Failed to normalize graph:", err);
      void showAlert("Graph normalization failed. See console for details.");
    }
  }, [nodes, edges, nodesToSpec, setSpec, showAlert]);
